...
Requires Plugins: nextend-facebook-connect
...
class NextendSocialLoginPRO {
    ...
    public static $nslMinVersion = '3.1.18';
            ...
            if (!is_user_logged_in()) {
                ...
                if (!defined('NSL_DISABLE_CUSTOM_ACTIONS')) {
                    ...
                    if ($useCustomRegisterLabel) {
                        $customActionsRegister = NextendSocialLogin::$settings->get('custom_actions_register');
                        if (!empty($customActionsRegister)) {
                            $customActionsRegisterArray = preg_split('/\r\n|\r|\n|,/', $customActionsRegister);
                            foreach ($customActionsRegisterArray as $customAction) {
                                add_action($customAction, 'NextendSocialLoginPRO::custom_actions_register_button_show');
                            }
                        }
                    }
                }
                ...
                if (!defined('NSL_DISABLE_CUSTOM_ACTIONS')) {
                    $customActionsLinkUnlink = NextendSocialLogin::$settings->get('custom_actions_link_unlink');
                    if (!empty($customActionsLinkUnlink)) {
                        $customActionsArray = preg_split('/\r\n|\r|\n|,/', $customActionsLinkUnlink);
                        foreach ($customActionsArray as $customAction) {
                            add_action($customAction, 'NextendSocialLoginPRO::custom_actions_link_unlink_button_show');
                        }
                    }
                }
    ...
    public static function validateSettings($newData, $postedData) {
        ...
        foreach ($postedData as $key => $value) {
            switch ($key) {
                ...
                case 'buddypress_registration_email_template':
                case 'custom_actions_button_style':
                case 'custom_actions_button_layout':
                case 'custom_actions_button_align':
                case 'custom_actions_register_button_style':
                case 'custom_actions_register_button_layout':
                case 'custom_actions_register_button_align':
                case 'custom_actions_link_unlink_button_align':
                ...
                case 'edd_form_button_align':
                    ...
                    break;
                case 'custom_actions':
                case 'custom_actions_register':
                case 'custom_actions_link_unlink':
    ...
    public static function custom_actions_register_button_show() {
        if (did_action('init')) {
            $index = NextendSocialLogin::$counter++;

            $containerID = 'nsl-custom-login-form-' . $index;

            echo '<div id="' . $containerID . '">' . NextendSocialLogin::renderButtonsWithContainer(NextendSocialLogin::$settings->get('custom_actions_register_button_style'), false, false, false, NextendSocialLogin::$settings->get('custom_actions_register_button_align'), 'register') . '</div>';

            $template = NextendSocialLogin::get_template_part('custom-actions/register/' . sanitize_file_name(NextendSocialLogin::$settings->get('custom_actions_register_button_layout')) . '.php');
            if (!empty($template) && file_exists($template)) {
                include($template);
            }
        }
    ...
    public static function custom_actions_link_unlink_button_show() {
        if (did_action('init')) {
            echo NextendSocialLogin::renderLinkAndUnlinkButtons('', true, true, NextendSocialLogin::$settings->get('custom_actions_link_unlink_button_align'));
        }
    }
    ...
    public static function pre_register_new_user_default() {
        ...
        if (class_exists('BuddyPress', false) && NextendSocialLogin::$settings->get('buddypress_registration_email_template') == 'buddypress') {
            remove_action('register_new_user', 'wp_send_new_user_notifications');
            add_action('register_new_user', 'NextendSocialLoginPRO::buddypress_send_new_user_notification');
        }
    }
    ...
    public static function wp_send_new_user_notifications_user($user_id) {
        $shouldSendDefaultNotification = true;

        if (class_exists('WooCommerce', false) && NextendSocialLogin::$settings->get('woocoommerce_registration_email_template') == 'woocommerce') {
            self::woocommerce_send_new_user_notification($user_id);
            $shouldSendDefaultNotification = false;
        }

        if (class_exists('BuddyPress', false) && NextendSocialLogin::$settings->get('buddypress_registration_email_template') == 'buddypress') {
            self::buddypress_send_new_user_notification($user_id);
            $shouldSendDefaultNotification = false;
        }
    ...
    public static function buddypress_send_new_user_notification($user_id) {
        if (class_exists('BuddyPress', false) && function_exists('bp_send_welcome_email')) {
            bp_send_welcome_email($user_id);
        }
    }
