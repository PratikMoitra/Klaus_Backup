...
class EDD_SL_Plugin_Updater {
	...
	private $update_check_ran     = false;
	...
	public function init() {
		...
		add_action( 'after_plugin_row', array( $this, 'show_update_notification' ), 1, 2 );
	...
	public function show_update_notification( $file, $plugin ) {
		...
		printf(
			...
			esc_html__( 'There is a new version of %1$s available.', 'wpo-updater' ),
			esc_html( $plugin['Name'] )
		);

		if ( ! current_user_can( 'update_plugins' ) ) {
			echo ' ';
			esc_html_e( 'Contact your network administrator to install the update.', 'wpo-updater' );
		} elseif ( empty( $update_cache->response[ $this->name ]->package ) && ! empty( $changelog_link ) ) {
			echo ' ';
			printf(
				/* translators: 1. opening anchor tag, do not translate 2. the new plugin version 3. closing anchor tag, do not translate. */
				__( '%1$sView version %2$s details%3$s.', 'wpo-updater' ),
			...
			echo ' ';
			printf(
				/* translators: 1,3. <a> tags, 2. version number, 4,5. </a> tags */
				__( '%1$sView version %2$s details%3$s or %4$supdate now%5$s.', 'wpo-updater' ),
				'<a target="_blank" class="thickbox open-plugin-details-modal" href="' . esc_url( $changelog_link ) . '">',
				esc_html( $update_cache->response[ $this->name ]->new_version ),
				'</a>',
			...
			printf(
				' %1$s%2$s%3$s',
				'<a target="_blank" class="update-link" href="' . esc_url( wp_nonce_url( $update_link, 'upgrade-plugin_' . $file ) ) . '">',
				esc_html__( 'Update now.', 'wpo-updater' ),
				'</a>'
			);
		}
	...
	public function show_changelog() {
		...
		if ( ! current_user_can( 'update_plugins' ) ) {
			wp_die( esc_html__( 'You do not have permission to install plugin updates', 'wpo-updater' ), esc_html__( 'Error', 'wpo-updater' ), array( 'response' => 403 ) );
		}
	...
	public function get_cached_version_info( $cache_key = '' ) {
		...
		if ( 'update-core.php' == $GLOBALS['pagenow']  && ! empty( $_REQUEST['force-check'] ) && false === $this->update_check_ran ) {
			$this->update_check_ran = true;
			return false;
		}
